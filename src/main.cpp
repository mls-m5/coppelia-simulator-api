...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 4) {
        ...
        printf("Indicate following arguments: 'leftMotorHandle "
               "rightMotorHandle sensorHandle'!\n");
#ifdef _WIN32
        Sleep(5000);
#else
        usleep(5000000);
    ...
    client.simxGetSimulationStepStarted(
        client.simxDefaultSubscriber(simulationStepStarted_CB));
    client.simxReadProximitySensor(
        sensorHandle, client.simxDefaultSubscriber(proxSensor_CB, 0));
    ...
    while (client.simxGetTimeInMs() - lastTimeReceived < 2000) {
        if (simTime - driveBackStartTime <
            3.0f) { // driving backwards while slightly turning:
            ...
            if (sensorTrigger > 0)
                driveBackStartTime = simTime; // We detected something, and
                                              // start the backward mode
        }
        client.simxSetJointTargetVelocity(
            leftMotorHandle, motorSpeeds[0], client.simxDefaultPublisher());
        client.simxSetJointTargetVelocity(
            rightMotorHandle, motorSpeeds[1], client.simxDefaultPublisher());
        client.simxSpinOnce();
        client.simxSleep(50);
    }
